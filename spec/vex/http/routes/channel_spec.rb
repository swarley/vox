...
RSpec.describe Vex::HTTP::Routes::Channel do
  let(:client) { Class.new { include Vex::HTTP::Routes::Channel }.new }
  ...
  describe '#create_message' do
    ...
    let(:file) do
      f = instance_double('Vex::HTTP::UploadIO')
    ...
    context 'when a file is being uploaded' do
      ...
      it 'sends a request with multipart attachments using numeric keys' do
        ...
        expect(client).to have_received(:request).with(
          route, data: hash_including(0 => instance_of(Vex::HTTP::UploadIO), :payload_json => '{}')
        )
      end
    end
  end
