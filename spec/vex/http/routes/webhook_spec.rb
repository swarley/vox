...
RSpec.describe Vex::HTTP::Routes::Webhook do
  let(:client) { Class.new { include Vex::HTTP::Routes::Webhook }.new }
  let(:id) { 'id' }
  let(:token) { 'token' }
  let(:content) { 'content' }
  ...
  describe '#execute_webhook' do
    ...
    context 'when a file is being uploaded' do
      let(:attachment_data) { hash_including(0 => instance_of(Vex::HTTP::UploadIO), :payload_json => '{}') }
      let(:file_data) { hash_including(file: instance_of(Vex::HTTP::UploadIO), payload_json: '{}') }
      let(:file_io) { Vex::HTTP::UploadIO.new(file) }
