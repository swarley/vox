...
require 'vex/http/routes'
require 'vex/http/error'
require 'vex/http/middleware'
...
module Vex
  ...
  module HTTP
    ...
    class Client
      ...
      DEFAULT_HEADERS = {
        'User-Agent': "DiscordBot (https://github.com/swarley/vex, #{Vex::VERSION})"
      }.freeze
      ...
      def inspect
        "#<Vex::HTTP::Client:0x#{object_id.to_s(16).rjust(16, '0')}>"
      end

      private

      # @param req [Faraday::Request] Request to be executed
      # @param route [Vex::HTTP::Route]
      ...
      def default_connection
        Faraday.new(url: API_BASE, headers: DEFAULT_HEADERS) do |faraday|
          faraday.use :vex_ratelimiter
          faraday.request :multipart
          faraday.response(
            :logger,
            Logging.logger['Vex::HTTP'],
            { formatter: Vex::HTTP::Middleware::LogFormatter }
          )
        end
      end
    end
